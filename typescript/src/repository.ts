...
export class PostgresRepository {
  ...
  async findSubscriptionById(id: string): Promise<SubscriptionRecord | null> {
    const { rows } = await this.pool.query<SubscriptionRecord>(
      `SELECT id, customer_id as "customerId", billing_type as "billingType", status, value, cycle, next_due_date as "nextDueDate", description, end_date as "endDate", max_payments as "maxPayments", created_at as "createdAt", updated_at as "updatedAt" FROM payment_subscriptions WHERE id=$1`,
      [id],
    );
    return rows[0] || null;
  }
  ...
  async findInvoiceByPaymentId(paymentId: string): Promise<InvoiceRecord | null> {
    const { rows } = await this.pool.query<any>(
      `SELECT id, payment_id as "paymentId", service_description as "serviceDescription", observations, value, deductions, effective_date as "effectiveDate", municipal_service_id as "municipalServiceId", municipal_service_code as "municipalServiceCode", municipal_service_name as "municipalServiceName", update_payment as "updatePayment", taxes_retain_iss as "taxesRetainIss", taxes_cofins as "taxesCofins", taxes_csll as "taxesCsll", taxes_inss as "taxesInss", taxes_ir as "taxesIr", taxes_pis as "taxesPis", taxes_iss as "taxesIss", status, payment_link as "paymentLink", created_at as "createdAt", updated_at as "updatedAt" FROM payment_invoices WHERE payment_id=$1 LIMIT 1`,
      [paymentId],
    );
    const row = rows[0];
    if (!row) {
    ...
    return {
      ...
      updatePayment: row.updatePayment,
      taxes: {
        retainIss: row.taxesRetainIss,
        cofins: row.taxesCofins,
        csll: row.taxesCsll,
        inss: row.taxesInss,
        ir: row.taxesIr,
        pis: row.taxesPis,
