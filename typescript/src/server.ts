...
async function bootstrap() {
  ...
  app.use((req, res, next) => {
    if (req.path.startsWith('/webhooks/asaas')) {
      return next();
    }
  ...
  const readRawBody = (req: express.Request): Promise<string> =>
  ...
  app.get('/payments', async (req, res) => {
    ...
    try {
      const payments = await service.listPaymentsByCustomer(customer);
      res.json(payments);
    } catch (err: any) {
      res.status(400).json({ error: err.message });
    }
  ...
  app.get('/subscriptions', async (req, res) => {
    ...
    if (!customer) return res.status(400).json({ error: 'customer é obrigatório' });
    try {
      const subscriptions = await service.listSubscriptionsByCustomer(customer);
  ...
  app.get('/invoices', async (req, res) => {
    ...
    if (!customer) return res.status(400).json({ error: 'customer é obrigatório' });
    try {
      const invoices = await service.listInvoicesByCustomer(customer);
      res.json(invoices);
    } catch (err: any) {
      res.status(400).json({ error: err.message });
    }
  ...
  app.all('/webhooks/asaas', async (req, res) => {
    if (req.method !== 'POST') {
      return res.status(405).json({ error: 'método não permitido' });
    }
    const accessToken = Array.isArray(req.headers['asaas-access-token'])
      ? req.headers['asaas-access-token'][0]
    ...
    try {
      ...
      return res.status(400).json({ error: 'não foi possível ler o corpo' });
    }
